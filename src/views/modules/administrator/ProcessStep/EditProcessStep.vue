								...
								:rules="[
									() =>
										m_editedItem.description.trim()
											.length <= 500 ||
										$t('message.maxlength_required_500'),
								]"
								dense
...
export default {
	...
	methods: {
		...
		async save() {
			...
			this.m_editedItem.stepName = this.m_editedItem.stepName.trim();

			this.m_editedItem.description =
				this.m_editedItem.description.trim();
			if (this.m_editedItem.stepName == "") {
				this.stepNameMessage = this.$t("message.msg_1");
				this.$refs["stepName"].focus();
				return;
			}
			if (this.isSaving) {
				return this.toastError(this.$t("message.msg_1"));
			}
			this.stepNameMessage = "";
			let response;
			...
			if (this.m_editedItem.description.trim().length > 500) {
				this.toastError(this.$t("error.max_length_500"));
