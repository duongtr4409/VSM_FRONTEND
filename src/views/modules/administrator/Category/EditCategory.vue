				...
				<v-row class="mt-2">
					<v-col cols="12">
						<v-form ref="form">
							...
							></v-textarea> -->
							<!-- <easyapi-json-editor
								:jsonData="m_CategoryGroupDto.description"
								v-model="m_CategoryGroupDto.description"
							></easyapi-json-editor> -->
							<div class="mb-4">
	...
	methods: {
		...
		async saveCategoryGroup() {
			if (!this.validRequiredText(this.m_CategoryGroupDto.name)) {
				this.toastError(this.$t("error.empty_categoryName"));
				return;
			}
			this.m_CategoryGroupDto.isActive = true;
			this.m_CategoryGroupDto.isDelete = false;
			...
			this.m_CategoryGroupDto.description = !this.m_CategoryGroupDto
				.description
				? {}
				: this.m_CategoryGroupDto.description;
			this.m_CategoryGroupDto.description = JSON.stringify(
				this.m_CategoryGroupDto.description,
			);
			if (
				!this.m_CategoryGroupDto.name ||
		...
		async m_update(value) {
			this.m_CategoryGroupDto = value;
			try {
				this.m_CategoryGroupDto.description = JSON.parse(
					this.m_CategoryGroupDto.description,
				);
			} catch (e) {
				// console.log(e);
			...
			if (!this.m_CategoryGroupDto || !this.m_CategoryGroupDto.id) {
				...
				// 		),
				// 	);
				// 	responseCate = await globalService.getData_Async(
