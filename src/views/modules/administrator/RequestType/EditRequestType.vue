					...
					<v-container>
						...
						<v-row>
							<v-col>
								<v-text-field
									maxLength="255"
									v-model="requestTypeDto.requestTypeCode"
									dense
									outlined
								>
									<template v-slot:label>
										{{ $t("label.request_type_code") }}
									</template>
								</v-text-field>
							</v-col>
						</v-row>
	...
	methods: {
		...
		onKeyUpDes() {
			if (
				...
				this.requestTypeDto.description.length < 500
			) {
				this.errorMessageDes = "";
			}
			if (this.requestTypeDto.description.length > 500) {
		...
		async save() {
			...
			this.isSaving = true;
			let response;
			this.requestTypeDto.requestTypeCode =
				this.requestTypeDto.requestTypeCode?.trim();
			this.requestTypeDto.requestTypeName =
