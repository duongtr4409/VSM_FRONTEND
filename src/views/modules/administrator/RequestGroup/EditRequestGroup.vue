					...
					<v-form v-model="valid">
						...
						<v-row>
							<v-col>
								<v-text-field
									maxLength="255"
									v-model="requestGroupDto.requestGroupCode"
									dense
									outlined
								>
									<template v-slot:label>
...
export default {
	...
	methods: {
		...
		onKeyUpDes() {
			...
			if (this.requestGroupDto.description.length > 500) {
		...
		async save() {
			...
			this.isSaving = true;
			let response;
			this.requestGroupDto.requestGroupCode =
						this.requestGroupDto.requestGroupCode?.trim();
			this.requestGroupDto.requestGroupName =
