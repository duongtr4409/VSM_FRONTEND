<template>
	<v-card elevation="0">
				...
				<v-card-text>
					<v-list-item-content class="p-10">
						<v-list-item-title class="mb-2"
							><strong>{{ $t("label.request_type_code") }}</strong
							>:
							<em>{{
						...
						>
						<v-list-item-title class="mb-2"
							><strong>{{ $t("label.request_type_name") }}</strong
							>:
							<em>{{
		...
		>
				...
				<v-tooltip bottom>
					<template v-slot:activator="{ on, attrs }">
						<span v-bind="attrs" v-on="on">
							{{ item.requestTypeName.slice(0, 30) }}
							<span v-if="item.requestTypeName.length > 20">
								...
							</span>
						</span>
					</template>
			...
			<template v-slot:[`item.requestTypeCode`]="{ item }">
				<v-tooltip bottom>
					<template v-slot:activator="{ on, attrs }">
						<span v-bind="attrs" v-on="on">
							{{ item.requestTypeCode.slice(0, 30) }}
							<span v-if="item.requestTypeCode.length > 20">
								...
							</span>
						</span>
					</template>
...
export default {
	...
	methods: {
		...
		messageRequestGroup() {
			if (this.editedItem.requestGroup) {
				return (this.errorMessageRequestGroup = "");
			}
		},
		onKeyUp() {
			if (
		...
		onKeyUpDes() {
			...
			if (this.editedItem.description.length > 500) {
				this.errorMessageDes = this.$t(
					"message.maxlength_required_500",
				);
			}
		},
		...
		async save() {
			try {
				...
				if (this.$refs?.form) {
					if (!this.$refs?.form.validate()) {
						return this.toastError(
							this.$t("error.please_fill_form"),
						);
					}
				}

				// if (this.editedItem.requestGroup.length > 255) {
				...
				if (this.editedIndex > -1) {
					this.editedItem.requestTypeCode =
						this.editedItem.requestTypeCode?.trim();
