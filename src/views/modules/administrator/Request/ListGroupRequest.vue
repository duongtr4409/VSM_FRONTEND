<template>
	<v-card elevation="0">
		...
		>
			...
			<template v-slot:[`item.requestGroupCode`]="{ item }">
				<v-tooltip bottom>
					<template v-slot:activator="{ on, attrs }">
						<span v-bind="attrs" v-on="on">
							{{ item.requestGroupCode.slice(0, 30) }}
							<span v-if="item.requestGroupCode.length > 20">
								...
							</span>
						</span>
					</template>
					<span>
						{{ item.requestGroupCode }}
					</span>
				</v-tooltip>
			</template>
		...
		async save() {
			try {
				...
				if (this.editedIndex > -1) {
					this.editedItem.requestGroupCode =
						this.editedItem.requestGroupCode?.trim();
