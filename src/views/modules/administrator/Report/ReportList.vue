				...
				<tr v-if="isFilter">
					...
					<td class="pl-0">
						<v-text-field
									...
									hide-details
									dense
									:placeholder="$t('label.requestDataCode')"
									maxLength="255"
								>
								</v-text-field>
					</td>
...
export default {
	...
	watch: {
		...
		requestsGroup(val) {
			if (val) {
				...
				this.getAllFileInFormOfRequest();
			}
			//thực hiện tìm kiếm theo đơn vị và loại yêu cầu luôn
			try {
				this.getAllRequestDataStatisticNoField();
			} catch (e) {
				console.error(e);
			}
		},
		list_Organizations(val) {
			console.log("val: ", val);
			//thực hiện tìm kiếm theo đơn vị và loại yêu cầu luôn
			try {
				this.getAllRequestDataStatisticNoField();
	...
	methods: {
		...
		/**
		 * Thống kê ko tìm kiếm theo trường dữ liệu
		 */
		async getAllRequestDataStatisticNoField() {
			if (
				!this.requestsGroup?.id &&
				(!this.list_Organizations ||
					(this.list_Organizations &&
						this.list_Organizations.length < 1))
			) {
				// nếu không có đơn vị và nhóm yêu cầu -> thống kê của cá nhân
				this.getRequestData();
			}

			if (!this.requestsGroup?.id) return; // nếu ko có requestsGroup?.id thì ko tìm kiếm
			try {
				let response;
				var requestbodyTemp = {
					requestId: this.requestsGroup.id,
					organizationIds:
						this.list_Organizations &&
						this.list_Organizations.length > 0
							? this.list_Organizations.map((e) => e.id)
							: this.listOrganizations.map((e) => e.id),
				};
				response = await globalService.postData_Async(
					administratorAPI.API_GetAllRequestDataStatistic(),
					requestbodyTemp,
