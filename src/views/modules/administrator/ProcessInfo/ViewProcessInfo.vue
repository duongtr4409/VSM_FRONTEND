		...
		<VuePerfectScrollbar>
			...
			<v-card-text class="white text--primary">
				<v-row>
					...
					<v-col cols="3" md="3">
						<h2>{{ $t("tab.general_information") }}</h2>
						<v-divider></v-divider>
						<v-list-item readonly v-model="viewProcess">
							<v-list-item-content>
								<v-list-item-title>
									<span :class="is_show_detail()">
										<strong>{{ $t("label.process_name") }}</strong
										>:
										<em>{{ viewProcess.processName }}</em>
									</span>
								</v-list-item-title>
								<span
									v-if="
						...
						<h2 v-if="stepInProcess.length > 0">
							{{ $t("label.step_info") }}
						</h2>
						<v-divider></v-divider>
						<v-card-text
