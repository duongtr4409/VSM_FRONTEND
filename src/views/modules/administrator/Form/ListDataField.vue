		...
		>
			<template v-slot:top>
				<v-toolbar flat>
					...
					<v-dialog v-model="dialogView" max-width="60vw" persistent>
						<v-card>
							<v-card-title>
								<span class="text-h5">{{ $t("title.information") }}</span>
							</v-card-title>
							<v-card-text>
								<v-container>
									<v-row>
					...
					<v-dialog v-model="dialogDelete" max-width="20vw" persistent>
						<v-card>
...
export default {
	...
	computed: {
		formTitle() {
			return this.editedIndex === -1 ? this.$t("label.add_new") : this.$t("label.edit");
		},
	},
	...
	methods: {
		...
		deleteAll() {
			if (this.selected.length > 0) {
				...
				alert(this.$t("message.alert_no_selected"));
			}
		},
