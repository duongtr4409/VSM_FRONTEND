<template>
	<v-card elevation="0" ref="form" color="primary">
		<v-toolbar dark color="primary" class="max-h-[2.5rem]">
			<v-toolbar-title class="mb-5"> {{$t("label.is_examine")}} </v-toolbar-title>
			<v-spacer></v-spacer>
			<v-btn icon dark @click="close" class="mb-5">
				...
				<v-form v-model="valid">
					<v-row>
						<v-col cols="12" sm="12" md="12">
							<v-textarea :label="$t('label.comment_note')" dense outlined>
							</v-textarea>
						</v-col>
						<v-col cols="12" sm="12" md="12">
							<div
							...
							>
								<div class="d-flex" v-if="steep.isActive">
									<h3 class="mt-2">{{$t("label.handler")}} :</h3>
		...
		<v-card-actions class="white">
			...
			<v-btn color="blue darken-1" text @click="close"> {{$t("label.deny")}} </v-btn>
		</v-card-actions>
	</v-card>
</template>
