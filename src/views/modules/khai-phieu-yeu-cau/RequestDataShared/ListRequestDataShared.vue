...
export default {
	...
	methods: {
		getRequestData(){
			...
			this.getAllRequestSharedToUserPagingNonAsync();
			this.getAllRequestSharedToUserNonAsync();
			this.getTotalCountData();
		},
		getTotalCountData(){
			globalService.getData_NoAsync(
				administratorAPI.API_CountRequestDataShareToUser(this.user_info.id)
			).then((response) => {
				if(!response ||
					response.status != 200 ||
					!response.data ||
		...
		getAllRequestSharedToUserNonAsync() {
		...
		getAllRequestSharedToUserPagingNonAsync() {
			this.requestTypes = [];
			globalService.getData_NoAsync(
				administratorAPI.API_GetRequestSharedToUserPaging(this.user_info.id, this.pageSize)
			).then((response) => {
				if(!response ||
					response.status != 200 ||
					!response.data ||
		...
		getFilenameExcel() {
			let str = "Phieu_duoc_chia_se_" + new Date().getTime();
			return str;
		},
	},
	filters: {
