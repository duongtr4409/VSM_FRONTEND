...
export default {
	...
	methods: {
		getRequestData() {
			...
			this.getAllRequestApprovedPagingNonAsync();
			this.getAllRequestApprovedNonAsync();
		...
		getAllRequestApprovedPagingNonAsync() {
			this.requestTypes = [];
			globalService
				.getData_NoAsync(
					administratorAPI.API_GetRequestApprovedPaging(
						this.user_info.id,
						this.pageSize,
					),
				)
		...
		getFilenameExcel() {
			let str = "Phieu_da_duyet_" + new Date().getTime();
			return str;
		},
	},
	filters: {
