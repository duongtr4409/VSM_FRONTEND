...
export default {
	...
	methods: {
		getRequestData(){
			...
			this.getAllRequestFollowPagingNonAsync();
			this.getAllRequestFollowNonAsync();
			this.getTotalCountData();
		},
		getTotalCountData(){
			globalService.getData_NoAsync(
				administratorAPI.API_CountRequestDataFollow(this.user_info.id)
			).then((response) => {
				if(!response ||
					response.status != 200 ||
					!response.data ||
		...
		getAllRequestFollowPagingNonAsync() {
			this.requestTypes = [];
			globalService.getData_NoAsync(
				administratorAPI.API_GetRequestFollowPaging(this.user_info.id, this.pageSize)
			).then((response) => {
				if(!response ||
					response.status != 200 ||
					!response.data ||
		...
		getFilenameExcel() {
			let str = "Phieu_theo_doi_" + new Date().getTime();
			return str;
		},
	},
	filters: {
