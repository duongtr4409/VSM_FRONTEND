				...
				<v-form v-model="valid">
					<v-row>
						...
						>
							...
							>
								<template v-slot:append-outer>
									<date-picker v-model="signDto.symbol" />
								</template>
							</v-text-field>
						...
						<v-col cols="12" sm="12" md="12">
							...
							/>
							<vue-perfect-scrollbar class="max-h-20">
								<v-list>
									<v-list-item
										v-for="file in files"
										:key="file.name"
...
export default {
	name: "VSMFEDigitalSignAllRequest",
	menu: false,
	props: { selected: [] },
	components: {},
	watch: {
	...
	data() {
		return {
			...
			list_form_digitalSignture: [
				...
				{
					id_DigitalSignture: SIGN_TYPE.SOFT,
					name_DigitalSignture: this.$t("label.sign_soft"),
				},
			],
			...
			tokenChoice: "", // biến đánh dấu kiểu ký token được chọn
		};
	},
	...
	methods: {
		...
		async approveRequest_All() {
			this.results = [];
			if (this.m_selected.length) {
				let responses = await this.sign(
					this.m_selected.map((ele) => ele.id),
				);
				if (!responses) {
					return
				}
					...
					for (const res of responses) {
						this.results.push({
							item: this.m_selected.find((entry) => {
								return entry.id == res.requestDataId;
							}),
							data: res.resultSign
								? this.$t("message.sign_success")
		...
		async sign(listId) {
			let response;
			this.signDto.reason = this.signDto.reason.trim();
			if (!this.signDto.reason) {
				this.toastError(this.$t("message.empty_requestContent"));
				return false
			}

			try {
				if (this.signDto.signType == SIGN_TYPE.SOFT) {
					this.signDto.requestDataList = listId;
					this.signDto.userId = this.user_info.id;
					response = await globalService.postData_Async(
						administratorAPI.API_SignAll(),
						this.signDto,
					);
					// console.log("this.results", this.results);
					if (!response) {
					...
					// this.$emit("close", true);
				} else if (this.signDto.signType == SIGN_TYPE.TOKEN) {
					let result = [];
					if (
						this.tokenChoice == SIGN_TYPE.SIGN_TYPE_TOKEN.MOBIFONE
					) {
						for (let requestDataId of listId) {
							let tempResult = {};
							tempResult.requestDataId = requestDataId;
							tempResult.resultSign =
								await this.signTokenMofiFone(requestDataId);
						...
						for (let requestDataId of listId) {
							let tempResult = {};
							tempResult.requestDataId = requestDataId;
							tempResult.resultSign = await this.signTokenViettel(
								requestDataId,
							);
							result.push(tempResult);
						}
					...
					return result;
				} else if (this.signDto.signType == SIGN_TYPE.SIM) {
					if (this.numberPhone) {
		...
		onClick(event) {
			event.preventDefault();
			let files = document.getElementById("filesDigSignAll");
			files.click();
		},
		remove(file) {
		...
		async saveReqdataProcessHis() {
			...
			try {
				...
				this.requestProcessHis.description = this.signDto.reason;
				...
				this.requestProcessHis.status = this.$t("label.signture");
				response = await globalService.postData_Async(
		...
		async getFiles() {
			...
			response = await globalService.getData_Async(
				administratorAPI.API_GetAllAttachmentFileByRequestDataIdHasSign(
		...
		async getFilesOfSignToken(requestDataId) {
			if (!requestDataId) requestDataId = this.m_requestData.id;
			let response;
			this.signFiles = [];
			response = await globalService.getData_Async(
				administratorAPI.API_GetAllAttachmentFileByRequestDataIdHasSign(
		...
		async signTokenMofiFone(requestDataId) {
			...
			await this.getFilesOfSignToken(requestDataId);
			for (let file of this.signFiles) {
				let res = await globalService.getData_Async(
					`/api/file/office365/hash-file/${file.id}`,
				);
				let dataSign = {
				...
				if (requestDataId) {
					this.signDto.requestDataId = requestDataId;
				} else {
		...
		async signTokenViettel(requestDataId) {
			...
			for (let file of this.signFiles) {
				...
				let signed;
				try {
					signed = await globalService.postDataOutside_Async(
