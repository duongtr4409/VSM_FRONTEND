...
export default {
	...
	data() {
		return {
			...
			file_name: [],
			rules: {
				isNumber: (value) => /^(0|[1-9][0-9]*)$/.test(value?.trim()) || !value || this.$t("message.please_enter_numeric_characters"),
			},
		};
	},
	...
	methods: {
		...
		async sign() {
			...
			try {
				if (this.signDto.signType == SIGN_TYPE.SOFT) {
					...
					// response = await globalService.postDataOutside_Async(
					...
					// }
					// let token = response;
					// let serialNumberRes = (response =
					// 	await globalService.postDataOutside_Async(
					// 		`http://localhost:8888/GetCert`,
					// 		{
					// 			token: token,
					// 		},
					// 	));
					...
					// 	);
					// }
					// let serialNumber = serialNumberRes;
					// await this.getFiles();
					...
					// 		`http://localhost:8888/SignHash`,
					// 		// `http://localhost:8888/SignXml`,
					// 		{
					// 			token: token,
					// 			data: res.data,
					...
					if (
						...
						await this.signTokenMofiFone();
					} else if (
						this.tokenChoice == SIGN_TYPE.SIGN_TYPE_TOKEN.VIETTEL
					) {
						await this.signTokenViettel();
					}
					// this.$emit("close", true);
					this.close();
		...
		onClick(event) {
			event.preventDefault();
			let files = document.getElementById("filesDigSign");
			files.click();
		},
		remove(file) {
		...
		async signTokenMofiFone() {
			...
			for (let file of this.signFiles) {
				...
				let res;
				try {
					res = await globalService.getData_Async(
						`/api/file/office365/hash-file/${file.id}`,
					);
					dataSign = {
