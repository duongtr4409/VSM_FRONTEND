					...
					<v-row>
						<v-col cols="12" sm="12" md="12">
							<v-textarea
								...
								outlined
								hide-details
								v-model="createRequestDto.description"
								maxLength="500"
							>
...
import CreateOtp from "@/models/createOtp.dto";
...
export default {
	...
	methods: {
		close() {
			...
			this.createRequestDto = Object.assign({}, this.defaultItem);
			this.files = [];
			this.file_name = [];
		},
		...
		async approveRequest() {
			try {
				...
				this.createRequestDto.description =
					this.createRequestDto.description.trim();
				if (!this.createRequestDto.description) {
					return this.toastError(
						this.$t("message.empty_requestContent"),
					);
				}
				this.createRequestDto.requestDataId = this.m_requestData.id;
				response = await globalService.postData_Async(
					administratorAPI.API_CreateOTP(
						this.createRequestDto.requestDataId,
						encodeURIComponent(this.createRequestDto.description),
					),
					{},
				);
				...
				if (response) {
					this.createRequestDto = Object.assign({}, CreateOtp);
				}
			} catch (error) {
		...
		onClick(event) {
			event.preventDefault();
			let files = document.getElementById("filesCreOTP");
			files.click();
		},
		remove(file) {
