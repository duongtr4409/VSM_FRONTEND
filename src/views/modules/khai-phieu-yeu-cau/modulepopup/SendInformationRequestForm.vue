...
import ExamineReplyDto from "@/models/examineReply.dto";
import ReqDataProcessHisDto from "@/models/req-data-process-his.dto";

export default {
	name: "VsmFeSendInformationRequestform",
	props: { request: {} },
	watch: {
		request(newValue) {
			this.m_requestData = newValue;
			this.examineReplyDto.content = "";
			this.examineReplyDto = Object.assign({}, ExamineReplyDto);
		},
	},
	...
	data() {
		return {
			file: null,
			m_requestData: null,
			examineReplyDto: ExamineReplyDto,
	...
	methods: {
		...
		async approveRequest() {
			try {
				this.examineReplyDto.content =
					this.examineReplyDto.content.trim();
		...
		onClick(event) {
			event.preventDefault();
			let files = document.getElementById("filesSendInfo");
			files.click();
		},
		remove(file) {
		...
		async saveReqdataProcessHis() {
			...
			try {
				...
				this.requestProcessHis.status = this.$t("label.reply_to_review");
				response = await globalService.postData_Async(
