	...
	methods: {
		...
		async approveRequest() {
			try {
				...
				if (!this.stepData && this.isDeny) {
					return this.toastError(
						this.$t("message.did_not_choose_to_refuse_step"),
					);
				}
				if (this.approveRequestDto.reason.length < 1) {
					return this.toastError(
