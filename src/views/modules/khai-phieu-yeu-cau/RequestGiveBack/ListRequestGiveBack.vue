				...
				<div class="d-flex items-center">
					<v-list-item>
						<vue-excel-xlsx
							...
							:columns="json_fields"
							:file-type="'xlsx'"
							:sheet-name="$t('label.return_request')"
						>
			...
			<!-- <template v-slot:[`item.actions`]="{ item }">
					<v-menu bottom left>
						...
						<v-list class="w-[120px]">
							...
							>
								<v-icon color="blue darken-2" class="pr-3">
									mdi-pencil-circle
								</v-icon>
								<v-list-item-title>
									{{ $t("button.edit") }}
...
export default {
	...
	methods: {
		getRequestData(){
			...
			this.getAllRequestGiveBackPagingNonAsync();
			this.getAllRequestGiveBackNonAsync();
			this.getTotalCountData();
		},
		getTotalCountData(){
			globalService.getData_NoAsync(
				administratorAPI.API_CountRequestDataGiveBack(this.user_info.id)
			).then((response) => {
				if(!response ||
					response.status != 200 ||
					!response.data ||
		...
		getAllRequestGiveBackPagingNonAsync() {
			this.requestTypes = [];
			globalService.getData_NoAsync(
				administratorAPI.API_GetRequestGiveBackPaging(this.user_info.id, this.pageSize)
			).then((response) => {
				if(!response ||
					response.status != 200 ||
					!response.data ||
		...
		getFilenameExcel() {
			let str = "Phieu_Tra_lai_" + new Date().getTime();
			return str;
		},
	},
	filters: {
